/* ═══════════════════════════════════════════════════════════
   ...
   Risk mapping  (+ signs → level 0–3):
     -       absent / no measurement  → skip
     +       alacsony (low)           → risk 0
     ++      közepes (medium)         → risk 1
     +++     magas (high)             → risk 2
     ++++    nagyon magas (very high) → risk 3
     n.a.    nincs adat               → skip
...
App.pdfParser = (function () {
  ...
  function plusToRisk(token) {
    ...
    if (t === '++')   return 1;
    if (t === '+')    return 0;
    return -1;  /* '-' or 'n.a.' → skip */
  }

  /* risk level → representative concentration (db/m³ equiv.) */
  const RISK_CONC = { 0: 5, 1: 20, 2: 60, 3: 200 };
